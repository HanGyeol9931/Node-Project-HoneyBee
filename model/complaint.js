...
class User extends Sql.Model{ // static init 메서드에서 테이블을 생성해주는건데 사용하면 테이블을 생성 및 연결까지(매핑) 구성 
    static init(sequelize){ // 상속받은 함수를 쓰려면 super 사용 // init함수의 첫번째 매개변수가 테이블의 구성
        ...
        return super.init(
            ...
            {
                sequelize,   // sequelize : 위에서 매개변수 쓴걸 연결 시켜주는 옵션, 이름은 무조건 sequelize설정
                // false로 했을때 
                ...
                tableName: "complaint", // 데이터 베이스의 테이블의 이름을 설정한다.
                paranoid : false, // paranoid true로 설정하면 deletedAt이라는 칼럼이 만들어집니다. 컬럼값은 남아있고 deleteAt이 값에 삭제한 시간이 추가된다.
                // charset, collate : 각각 밑에 처럼 설정해주면 한글이 입력 가능하게 되고. // 이모티콘같은것도 쓸려면 utf-8 뒤에 mb4붙이면댐
                charset: "utf8mb4",
