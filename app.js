  ...
  User.findOne({
    ...
    .then((e) => {
      ...
      if ((userid && nickname) == "") {
        // 유저아이디와 패스워드가 공란이라면
        res.send(
          '<script type="text/javascript">alert("아이디와 닉네임을 입력해주세요."); location.href="/";</script>'
        );
      } else if (e === null) {
        res.send(
          '<script type="text/javascript">alert("없는정보입니다"); location.href="/";</script>'
        ...
        compare(userid, nickname, (err, same) => {
          if (same) {
            ...
            res.send(
              '<script type="text/javascript">alert("아이디와 닉네임이 일치하지 않습니다."); window.location.href="/findPw";</script>'
            );
          }
        });
      }
